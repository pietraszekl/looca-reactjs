...
class ProjectDetails extends React.Component {
  ...
  render() {
    let ProjectLinksList;
    const project_id = this.props.project;
    const project_data = this.state.data.projects.list[project_id];

    if (project_data.links) {
      ProjectLinksList = project_data.links.map((link, i) => (
        <ProjectDetailsLink key={i} link={link} />
      ));
    }
    const ProjectTechList = project_data.tech.map((tech, i) => (
      <ProjectDetailsTech key={i} tech={tech} />
    ));

    const ProjectImages = project_data.img.map((image, i) => (
      <ProjectDetailsImages key={i} image={image} />
    ));
